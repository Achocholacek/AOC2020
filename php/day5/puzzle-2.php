...
sort($takenArray);
$arrSize = count($takenArray);
$loop = 0;
while ($loop < $arrSize) {
    $curr = $takenArray[$loop];
    $next = $takenArray[$loop + 1];
    if (($next - $curr) != 1 && isset($next)) {
        echo "Your Seat ID is between " .  $curr . " and " . $next . "\n";
    }
    $loop++;
}
...
//echo parseSeatString('BBBBBBBRRR') . " S/B 820\n";